...
export class MensagensComponent implements OnInit, OnDestroy {
  ...
  private setupChatTopic() {
    if (this.selectedPatient?.id && this.currentUser?.id) {
      const minId = Math.min(this.currentUser.id, this.selectedPatient.id);
      const maxId = Math.max(this.currentUser.id, this.selectedPatient.id);
      this.chatTopic = `/topic/chat/${minId}/${maxId}`;
    ...
    if (this.chatTopic) {
      ...
      this.messageSubscription = this.apiService.messageSubject.subscribe(
        (message: ChatMessage) => {
          ...
          const isFromSelectedPatient =
            this.selectedPatient &&
            message.sender === this.selectedPatient.username;
          const isFromPhysiotherapist =
            this.currentUser?.fisioterapeuta &&
            message.sender === this.currentUser.fisioterapeuta.username;
          const isToCurrentUser =
            message.recipient === this.currentUser?.username;
          ...
          if (this.currentUser?.perfil === 'TECHNICAL') {
            ...
            const isBetweenCurrentPhysioAndSelectedPatient =
              (isFromCurrentUser &&
                message.recipient === this.selectedPatient?.username) ||
  ...
  toggleChat() {
    ...
    if (this.isOpen) {
      ...
      if (
        this.currentUser?.perfil === 'TECHNICAL' &&
